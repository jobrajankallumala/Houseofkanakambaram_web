    ...
    return (
        <Row className="text-center mt-5 pt-4">
            <Col className="px-0 mx-3" style={{ 'overflow': 'hidden' }}>
                <h3 className="h2 mb-5">Trending</h3>
                <Carousel responsive={responsive}
                    partialVisible={true}
                    containerClass="product-items mb-5 row">
                    {items.length ? items.map((item, index) => {
                        return <Item key={index} data={item}  filter="filter_by=created_at&order_by=desc&tag_string=trending" />
                    }) : <div></div>}
                </Carousel>
                <Row className="justify-content-center">
                    <Link to="/search?filter_by=created_at&order_by=desc&tag_string=trending" className=" btn-default">See All <BsArrowRight /></Link>
                </Row>
            </Col>
